#!/usr/bin/env python3
"""
Noticing Game - Windows Executable Builder

This script helps you build a standalone executable for the Noticing Game desktop application
using PyInstaller on Windows. It handles dependency checking, environment setup, and building process.

Usage:
    python build_executable_windows.py [options]
...
def print_banner():
    ...
    print("=" * 70)
    print("🎮 NOTICING GAME - WINDOWS EXECUTABLE BUILDER")
    print("=" * 70)
...
def check_platform():
    ...
    if platform.system() != 'Windows':
        ...
        print("   For Linux/macOS, use build_executable_linux.py")
        return False
...
def create_spec_file(debug=False, onedir=False):
    ...
    print("Creating PyInstaller spec file for Windows...")

    project_root = find_project_root()
    backend_dir = project_root / "backend"
    spec_file = backend_dir / 'desktop_app_windows.spec'
...
exe = EXE(
    ...
    if assets_icon.exists():
        icon_data_path = str(assets_icon).replace('\\', '/')
        data_files.append(f"(r'{icon_data_path}', '.'),")
    elif backend_icon.exists():
        data_files.append("(r'icono.ico', '.'),")

    spec_content = f'''# -*- mode: python ; coding: utf-8 -*-
# Windows-specific PyInstaller spec file for Noticing Game
...
a = Analysis(
    ...
    binaries=[],
    datas=[
        {chr(10).join("        " + line for line in data_files)}
    ],
    hiddenimports=[
        'flask',
        'flask_cors',
...
'''
    ...
    print(f"✅ Windows spec file created: {spec_file}")
    return spec_file
...
def test_executable(build_root=True, use_distribution_dir=False):
    ...
    if (dist_dir / 'noticing_game_server.exe').exists():
        # One-file bundle
        executable = dist_dir / 'noticing_game_server.exe'
    elif (dist_dir / 'noticing_game_server' / 'noticing_game_server.exe').exists():
        # One-directory bundle
        executable = dist_dir / 'noticing_game_server' / 'noticing_game_server.exe'
...
def show_build_results(build_root=True, use_distribution_dir=False):
    ...
    print("\n📦 DISTRIBUTION:")
    if (dist_dir / 'noticing_game_server.exe').is_file():
        print("   Single file executable - distribute the .exe file only")
    else:
    ...
    print("\n⚠️  NOTES:")
    print("   - First run may be slower as Windows scans the executable")
...
def check_windows_tools():
    """Check for Windows-specific development tools"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("⚠️  Visual Studio Build Tools: Not found (optional)")
    except:
        print("⚠️  Visual Studio Build Tools: Not found (optional)")
    ...
    if sdk_found:
        ...
        print("⚠️  Windows SDK: Not found (optional)")

def main():
    """Main build function"""
    parser = argparse.ArgumentParser(description="Build Noticing Game Windows executable")
    ...
    parser.add_argument("--check-tools", action="store_true", help="Check Windows development tools")
    ...
    print("You can now distribute the executable to run Noticing Game on Windows")
