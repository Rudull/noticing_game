    ...
    function showPanelWithRetry() {
        chrome.tabs.query(
            ...
            function (tabs) {
                ...
                // Verificar que estamos en YouTube o Netflix o Disney+
                const isYouTube = activeTab.url.includes("youtube.com/watch");
                const isNetflix = activeTab.url.includes("netflix.com/watch");
                const isDisney = activeTab.url.includes("disneyplus.com");

                if (!isYouTube && !isNetflix && !isDisney) {
                    alert(
                        "You must be on a YouTube, Netflix or Disney+ video page to use this extension.",
                    );
                    return;
                }
