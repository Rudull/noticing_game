...
window.UIComponents = (function () {
  ...
  function createLearnedWordsModal(learnedWords, onClose) {
    ...
    if (window.ActivityHeatmap && window.WordDifficultyTracker && typeof window.WordDifficultyTracker.getDailyStats === 'function') {
      const dailyStats = window.WordDifficultyTracker.getDailyStats() || {};
      const heatmap = window.ActivityHeatmap.create(dailyStats);
      // Insert heatmap after header, before subheader
      modal.insertBefore(heatmap, subHeader);
    }
  ...
  // Crear modal de previsualización de palabras de una lista
  function createWordsPreviewModal(listName, words, onClose) {
    // Eliminar cualquier modal anterior
    const existingModal = document.querySelector(".learned-words-modal"); // Reusamos la clase por estilo
    if (existingModal && existingModal.parentNode) {
      existingModal.parentNode.removeChild(existingModal);
    }

    const modal = document.createElement("div");
    modal.className = "learned-words-modal"; // Reusamos estilos
    ...
    titleText.className = "learned-words-modal-title";
    titleText.textContent = listName;

    const subtitleText = document.createElement("small");
    subtitleText.style.fontSize = "12px";
    subtitleText.style.color = "var(--secondary-text)";
    subtitleText.style.fontWeight = "normal";
    subtitleText.textContent = `${words.length} words`;

    title.appendChild(titleText);
    title.appendChild(subtitleText);
    ...
    // Lista
    const listContainer = document.createElement("div");
    listContainer.className = "learned-words-list";
    listContainer.style.maxHeight = "300px";
    listContainer.style.overflowY = "auto";

    if (!words || words.length === 0) {
      listContainer.textContent = "No words in this list.";
      listContainer.style.fontStyle = "italic";
      listContainer.style.color = "var(--secondary-text)";
      ...
      words.forEach(item => {
        ...
        if (typeof item === 'object' && item !== null && item.word) {
          ...
          wordText = String(item);
        }

        const spanWord = document.createElement("span");
        spanWord.textContent = wordText;
        li.appendChild(spanWord);

        if (difficulty !== null) {
          const spanDiff = document.createElement("span");
          spanDiff.textContent = `Diff: ${difficulty.toFixed(1)}`;

          // Color coding
          if (difficulty <= 2) spanDiff.style.color = "var(--success-color)";
          else if (difficulty <= 5) spanDiff.style.color = "orange";
          else spanDiff.style.color = "var(--error-color)";

          spanDiff.style.fontSize = "12px";
          spanDiff.style.fontWeight = "bold";
