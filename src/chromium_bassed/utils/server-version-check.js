...
const ServerVersionCheck = (function () {
    ...
    const MIN_SERVER_VERSION = "0.1.3";
...
`;
    ...
    // Muestra advertencia si el servidor no está disponible (Offline/No instalado)
    function showServerOfflineModal() {
        console.log("ServerVersionCheck: Showing server offline modal");
        ...
        icon.innerHTML = "&#9888;";
        icon.style.fontSize = "32px";
        icon.style.color = "#f44336"; // Red for error
        icon.style.marginBottom = "12px";
        icon.style.textAlign = "center";
        modal.appendChild(icon);
        ...
        const mainMsg = document.createElement("div");
        mainMsg.style.marginBottom = "8px";
        mainMsg.innerHTML = `<b>Subtitle Server is not running.</b>`;
        modal.appendChild(mainMsg);
        ...
        warnMsg.innerHTML = `
        <span>
            The companion server required for extracting subtitles from YouTube is not detected.
            <br><br>
            Please ensure you have downloaded and launched the <b>noticing_game_server</b> executable.
        </span>
    `;
        modal.appendChild(warnMsg);
        ...
        linkDiv.innerHTML = `
        <a href="https://github.com/Rudull/noticing_game" target="_blank" style="color: #6544e9; text-decoration: underline; font-weight: bold;">
            Download / Instructions on GitHub
        </a>
    `;
        modal.appendChild(linkDiv);
        ...
        const style = document.createElement("style");
        style.textContent = `
        :root { --ng-modal-bg: #fff; --ng-modal-fg: #222; }
        @media (prefers-color-scheme: dark) { :root { --ng-modal-bg: #23272e; --ng-modal-fg: #f1f1f1; } }
        `;
        document.head.appendChild(style);
    }
