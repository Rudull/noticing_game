...
window.YouTubeIntegration = (function () {
  ...
  function createToggleButton(callback) {
    ...
    // Crear interruptor de blur de subtítulos
    const blurToggle = createBlurToggleButton(player);
    ...
    function showToggle() {
      ...
      if (blurToggle) {
        ...
        blurToggle.style.pointerEvents = "auto";
      }
    }
    ...
    function hideToggle() {
      ...
      if (video && !video.paused && !video.ended) {
        ...
        if (blurToggle) {
          blurToggle.style.opacity = "0";
          blurToggle.style.pointerEvents = "none";
        }
      }
    }
    ...
    pauseToggleBtn.addEventListener("click", function () {
      ...
      chrome.storage.local.set(
        ...
        function () {
          ...
          // Mostrar mensaje de confirmación usando helper robusto
  ...
  // Crear interruptor flotante para blur de subtítulos
  function createBlurToggleButton(player) {
    if (document.querySelector(".noticing-game-blur-toggle")) {
      return document.querySelector(".noticing-game-blur-toggle");
    }

    let isBlurEnabled = false;

    const blurToggleBtn = document.createElement("button");
    blurToggleBtn.className = "noticing-game-blur-toggle";
    blurToggleBtn.setAttribute("title", "Toggle Blur Subtitles");
    ...
    chrome.storage.local.get(["blurSubtitles"], function (result) {
      ...
      blurToggleBtn.innerHTML = getBlurSwitchSVG(isBlurEnabled);
    });
    ...
    Object.assign(blurToggleBtn.style, {
      position: "absolute",
      top: "calc(50% + 104px)", // Debajo del interruptor de pausa
      left: "auto",
      right: "12px",
      transform: "translateY(-50%)",
      zIndex: "10010",
    ...
    player.appendChild(blurToggleBtn);

    return blurToggleBtn;
  }
