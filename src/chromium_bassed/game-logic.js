        ...
        if (typeof chrome !== "undefined" && chrome.storage) {
            ...
            chrome.storage.sync.get(["wordClicksToOvercome"], function (syncResult) {
                if (!chrome.runtime.lastError && syncResult.wordClicksToOvercome) {
                    let value = parseInt(syncResult.wordClicksToOvercome);
                    ...
                    chrome.storage.local.get(["wordClicksToOvercome"], function (result) {
                        ...
                        if (!value || value < 1 || value > 6) value = 3;
                        CLICKS_TO_REPLACE_WORD = value;
                        // Auto-migrate to sync for next time
                        chrome.storage.sync.set({ wordClicksToOvercome: value });
                        if (typeof callback === "function") callback(value);
                    });
                }
            });
        ...
        if (typeof chrome !== "undefined" && chrome.storage) {
            ...
            chrome.storage.sync.get(["gridColumns", "gridRows"], function (syncResult) {
                if (!chrome.runtime.lastError && (syncResult.gridColumns || syncResult.gridRows)) {
                    const columns = syncResult.gridColumns || 5;
                    const rows = syncResult.gridRows || 5;
                    const totalWords = columns * rows;
                    if (callback) {
                        callback({ columns, rows, totalWords });
                    }
                } else {
                    // Fallback to local
                    chrome.storage.local.get(["gridColumns", "gridRows"], function (result) {
                        ...
                        const totalWords = columns * rows;
                        // Migrate
                        chrome.storage.sync.set({ gridColumns: columns, gridRows: rows });
                        if (callback) {
                            callback({ columns, rows, totalWords });
                        }
                    });
