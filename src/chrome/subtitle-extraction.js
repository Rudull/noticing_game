...
window.SubtitleExtraction = (function () {
  ...
  function showServerSetupGuide() {
    ...
    const downloadLink =
      "https://github.com/Rudull/noticing_game/releases/tag/Noticing-Game-Server-V0.1.0";
...
Once the server is running, refresh this page and you can use the extension normally.
  ...
  async function getYouTubeSubtitles() {
    try {
      ...
      if (!isServerOnline) {
        console.log("Backend server is offline, showing setup guide...");
      ...
      if (!response.ok) {
        if (response.status === 400) {
          ...
          throw new Error(errorData.error || "Bad request to backend");
        }
        throw new Error(`Backend server error: ${response.status}`);
      }
      ...
      if (!data.success) {
        ...
        if (data.error && data.error.includes("No subtitles available")) {
          alert(
                ...
                window.TextProcessing.processSubtitleTextForWordDetection(
                  ...
                  function (word) {
                    ...
                    if (window.WordDetection) {
                      ...
                      const timestamp = Math.floor(subtitle.start * 1000);
                      window.WordDetection.trackWordAppearance(word, timestamp);
                      console.log(
              ...
              window.TextProcessing.processSubtitleTextForWordDetection(
                ...
                function (word) {
                  ...
                  if (window.WordDetection) {
                    ...
                    console.log(`Word detected (fallback): ${word}`);
                  }
                },
              );
            }
