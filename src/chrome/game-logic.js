...
window.GameLogic = (function () {
  ...
  function handleCorrectClick(wordButton, wordInfo, timeDiff) {
    ...
    // Actualizar el texto del botón para mostrar el contador
    wordButton.textContent = `${wordInfo.word} (${newClicks})`;
  ...
  function handleIncorrectClick(wordButton, wordInfo, statusElement) {
    ...
    if (currentClicks > 0) {
      ...
      wordButton.textContent = wordInfo.word; // Quitar contador de clics
      console.log(
  ...
  function createWordButton(wordInfo, index, container, statusElement) {
    ...
    // Mostrar "i" en mayúscula
    const wordText = wordInfo.word.toLowerCase() === "i" ? "I" : wordInfo.word;
